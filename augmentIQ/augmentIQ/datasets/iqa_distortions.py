...
if __name__ == '__main__':
    ...
    # Load the image
    img_path = '/home/liilu/Desktop/COURSE/Pic_proc/AIGC/BASELINE/augmentIQ/augmentIQ/sample_images/10004473376.jpg'
    img = Image.open(img_path)

    # Define the transforms
    transforms_list = [
        lambda x: imjitter(x, 4),
        lambda x: imblurgauss(x, 4),
        lambda x: imblurlens(x, 4),
        lambda x: imblurmotion(x, 4),
        lambda x: imcolordiffuse(x, 4),
        lambda x: imcolorshift(x, 4),
        lambda x: imcolorsaturate(x, 4),
        lambda x: imsaturate(x, 4),
        lambda x: imcompressjpeg(x, 4),
        lambda x: imnoisegauss(x, 4),
        lambda x: imnoisecolormap(x, 4),
        lambda x: imnoiseimpulse(x, 4),
        lambda x: imnoisemultiplicative(x, 4),
        lambda x: imdenoise(x, 4),
        lambda x: imbrighten(x, 4),
        lambda x: imdarken(x, 4),
        lambda x: immeanshift(x, 4),
        lambda x: imresizedist(x, 4),
        lambda x: imresizedist_bilinear(x, 4),
        lambda x: imresizedist_nearest(x, 4),
        lambda x: imresizedist_lanczos(x, 4),
        lambda x: imsharpenHi(x, 4),
        lambda x: imcontrastc(x, 4),
        lambda x: imcolorblock(x, 4),
        lambda x: impixelate(x, 4),
        lambda x: imnoneccentricity(x, 4),
        # lambda x: imwarpmap(x, np.random.randn(x.size[0], x.size[1], 2)),
        lambda x: imjitter(x, 4),
    ]

    augmented_images = []
    augmented_images.append(torch.from_numpy(np.array(img)))
    for transform in transforms_list:
        # Apply the transform to the tensor
        print(img.size)
        transformed_img = transform(img)
        logger.info(f"The transformed img is of shape {transformed_img.size}")
        # Check that the transformed image is the same size as the original image
        assert transformed_img.size == img.size, "Transformed image size does not match original image size"

        # Append the transformed image to the list of augmented images
        augmented_images.append(torch.from_numpy(np.array(transformed_img)))

        # Check that the transformed image is a PIL image
        assert isinstance(transformed_img, Image.Image), "Transformed image is not a PIL image"

    # Check that the original image is a PIL image
    assert isinstance(img, Image.Image), "Original image is not a PIL image"

    # Concatenate the augmented images into a single tensor
    concatenated_image = torch.stack(augmented_images, dim=0)

    # Convert the tensor to a numpy array and transpose the dimensions
    concatenated_image = concatenated_image.numpy()
    ...
    # Plot the original image and the augmented images in a grid
    for i in range(rows):
        for j in range(cols):
            index = i * cols + j
            if index >= concatenated_image.shape[0]:
                break
            ax = fig.add_subplot(rows, cols, index + 1)
            ax.axis("off")
            ax.imshow(concatenated_image[index])
            print(index)
            if i + j == 0:
