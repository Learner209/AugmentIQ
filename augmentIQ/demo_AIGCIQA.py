...
from augmentIQ.datasets.iqa_dataset import AIGCIQA_2023, build_AIGCIQA
import copy
import os
import random
...
class IQA_trainer(object):
    ...
    def _build_model(self):
        ...
        self.content_aware_net = ContentAwareFeats(ckpt_path=self.args.content_aware_ckpt_path, device=self.device)
    ...
    def train(self, setting):
        train_data, train_loader = build_AIGCIQA(self.args, self.text_alignment_tokenizer, flag='train')
        ...
        best_model_cards = self.model_cards
    ...
    def _process_one_batch(self, batch_meta):
        ...
        batch_mos_quality_data = F.normalize(batch_mos_quality_data, dim=0).reshape(-1, 1)
        ...
        batch_mos_correspondence_data = batch_meta["mos_correspondence_data"].float().to(self.device)
        batch_mos_correspondence_data = F.normalize(batch_mos_correspondence_data, dim=0).reshape(-1, 1)
        ...
        batch_std_authenticity_data = batch_meta["std_authenticity_data"].float().to(self.device)
        batch_std_authenticity_data = F.normalize(batch_std_authenticity_data, dim=0).reshape(-1, 1)
        ...
        # logger.info(f"The MSE loss between feat and batch_mos_quality_data is {nn.MSELoss()(feat, batch_mos_quality_data + batch_mos_authenticity_data)}")
        return feat, batch_mos_quality_data + batch_mos_authenticity_data, text_alignment_feat, batch_mos_correspondence_data
        # return feat, batch_mos_quality_data + batch_mos_authenticity_data, text_alignment_feat, batch_mos_correspondence_data

    def test(self, setting, fold=0):
        test_data, test_loader = build_AIGCIQA(self.args, self.text_alignment_tokenizer, flag='test')
...
if __name__ == "__main__":
    ...
    logger.info(args)
    args.aug = True
    setting = "AIGCIQA_bsz_{}_n_args_{}_epochs_{}_aug_{}".format(
